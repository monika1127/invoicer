...
class InvoiceDetails extends Component {
    ...
    async componentDidMount() {
        try {
            const invoiceResponse = await fetch(`http://localhost:5000/invoices/${this.props.match.params.id}`)
            const invoice = await invoiceResponse.json()
            const contractorResponse = await fetch(`http://localhost:5000/contractors?companyName=${invoice.contractor}`)
            const [contractorInfo] = await contractorResponse.json()
            this.setState({ invoice, contractorInfo, isLoading: false })
        } catch (err) {
            //alert error
            console.log(err)
        }
